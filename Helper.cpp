...
Menu *Helper::parseMenus(const QString &path)
{
    ...
    if (parseJson(path, doc)) {
        ...
        json["title"].isNull() ? mainMenu->setTitle("Main Menu") : mainMenu->setTitle(json["title"].toString());
        json["controllerName"].isNull() ? mainMenu->setControllerName("DefaultController")
                                        : mainMenu->setControllerName(json["controllerName"].toString());
        json["controllerMode"].isNull() ? mainMenu->setControllerMode(-1) : mainMenu->setControllerMode(json["controllerMode"].toInt());
...
QList<Contact *> Helper::parseContacts(const QString &path)
{
    ...
    if (parseJson(path, doc)) {
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            object["name"].isNull() ? contact->setName("null") : contact->setName(object["name"].toString());
            object["phoneNumber"].isNull() ? contact->setPhoneNumber("null") : contact->setPhoneNumber(object["phoneNumber"].toString());
            contacts << contact;
        }
    }
...
Menu *Helper::parseMenus(QJsonObject object, Menu *parent, int index)
{
    ...
    object["controllerMode"].isNull() ? menu->setControllerMode(-1) : menu->setControllerMode(object["controllerMode"].toInt());
